...
load (const char *file_name, void (**eip) (void), void **esp) 
{
	...
	char *save_ptr;
	int argc=0;
	uint32_t addr_argv[argc];
	uintptr_t tp=(uintptr_t) *esp;
	...
	///todo_prj2_10_30:1:37//////////////////////////////
	//1.arv[][]넣는 과정
	for(i=argc-1; i>=0; i--)
	{
		unsigned int len_argv= strlen(argv[i]) +1; //NULL길이까지
		*esp -= len_argv;
		strlcpy(*esp,argv[i],len_argv);
	...
	unsigned int word_gap=(unsigned int)tp2-tp;
	*esp-= word_gap %4;
	memset(*esp,0,word_gap);
