...
process_wait (tid_t child_tid UNUSED) 
{
	...
	if( contain == false)
		return -1;
	if( cur -> exit_accpeted ==1)
		return -1;
	if( cur -> waitfor == child_tid)
		return -1;

	cur -> waitfor= child_tid;
	sema_down( &tp->sema_wait);
...
load (const char *file_name, void (**eip) (void), void **esp) 
{
	...
	char *token[100];
        char *save_ptr;
	int cnt=0;
	//for (token[cnt] = strtok_r (file_name," ", &save_ptr); token[cnt]!= NULL;token[++cnt] = strtok_r (NULL," ", &save_ptr));
	...
	for(i=cnt-1;i>=0;i--){
		*esp-=(strlen(token[i])+1);
		strlcpy(*esp,token[i],strlen(token[i])+1);
		address[i]=*esp;
	}
	
	//2.word_align 
	*esp-=1;
	//uint8_t* temp=(uint8_t*)*esp;
	//*temp=0;//word align
	*(uint8_t *) *esp= 0;
