...
//		printf("wiat함수 1찾는다\n");
		...
		{
//			printf("ABABAB");
			while(parent->status !=THREAD_RUNNING)
			{
			}
			contain_flag=1;
			break;
...
//		printf("없어\n");
	...
	//B_case) 이미 죽은걸 accepted twice check 
...
//		printf("이미확인함\n");
	...
	if(tp->waitfor ==child_tid){
//		printf("waited twice\n");
		return -1;
	}


	//C_case) kernel등에 의한 죽음 /child가 lock걸려있을때 
	if(tp->status == THREAD_BLOCKED)
	{
		thread_unblock(tp);
		intr_disable();
		thread_block();
