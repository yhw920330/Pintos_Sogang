               ...
               thread_func *function, void *aux) 
{
  ...
  ///prj2_1 2015:10:32 14:59 /update !! 이걸써줘야된대 (동기가)////
   struct thread *p = thread_current(); //만들당시의 current는 만들어질 th의 부모
   struct list_elem *e;
   t->parent = p;  //부모 알려주고
   /////////////////fail of 쓰레기값
   //t->exit_status =-1;
   list_init ( &p->child); //초기화 하고   ->childd가 여러개일때는 없어야해
   list_push_back( &(p->child), &(t->gene)); //부모의 child list마지막에 날 포함
   list_init(&(t->child));
   //!!!!//t->exit_status = -1;// 초기값 뭘로 해야 적절할까? 
   t->exit_accepted =0;
  //////--------------------------------////
